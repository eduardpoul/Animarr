    ...
    ILogger<MetadataService> logger)
{
        ...
        CancellationToken ct = default)
    {
        ...
        // For flat files use the filename without extension as title source
        var titleSource = folder.SingleFilePath != null
            ? Path.GetFileNameWithoutExtension(folder.SingleFilePath)
            : folder.Path;
        var searchTitle = llmTitle ?? ParseTitleFromPath(titleSource);
        var folderYear  = ExtractYearFromPath(titleSource);
