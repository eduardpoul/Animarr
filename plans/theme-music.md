...
> **✅ Реализовано (2026-05-29).** Все 5 шагов в ветке, сборка зелёная (0 ошибок),
> контракты AnimeThemes/AniList проверены живыми ответами. Хранилище — `.animarr`
> рядом с медиа (§3). Два уточнения против исходного плана:
> 1. Раздача `GET /api/media/{id}/theme` делает **ленивую дозагрузку** при промахе
>    кеша → существующая библиотека покрывается без полного reidentify (§5/§8).
> 2. Префетч на этапе идентификации **гейтится** на «хоть один юзер включил тему» —
>    дефолтный импорт не тормозит, opt-in юзер получает мгновенное проигрывание.
